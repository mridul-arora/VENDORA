...
public class SettingsActivity extends AppCompatActivity {
    ...
    private void uploadProfilePicture() {
        ...
        if (imageUri != null) {
            final StorageReference fileRef = storageProfilePicRef.child(Objects.requireNonNull(mAuth.getCurrentUser()).getUid() + ".jpg");
    ...
    private void validateAndSaveOnlyInformation() {
           ...
           else
        {
            ...
            HashMap<String, Object> userMap = new HashMap<>();
            userMap.put("uid", Objects.requireNonNull(mAuth.getCurrentUser()).getUid());
            userMap.put("name", nameEditText.getText().toString());
            userMap.put("phone", phoneEditText.getText().toString());
    ...
    databaseReference.child(Objects.requireNonNull(mAuth.getCurrentUser()).getUid()).addValueEventListener(new ValueEventListener() {
        ...
        public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
            if (dataSnapshot.exists() && dataSnapshot.getChildrenCount() > 0) {
                ...
                if (getType.equals("Vendors")) {
                    String cart = Objects.requireNonNull(dataSnapshot.child("cart").getValue()).toString();
                    vendorCartNumber.setText(cart);
                    mService = Objects.requireNonNull(dataSnapshot.child("service").getValue()).toString();
